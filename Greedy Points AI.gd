	...
	for board in potentialBoards:
		...
		for y in range(len(board.state[0])):
			...
			for x in range(len(board.state)):
				if board.state[x][y][0] == 0:
					full = false
				elif board.state[x][y][0] == 2: 
					heightScore += y
					if y + 1 < 20:
						if board.state[x][y + 1][0] == 0: holeCount += 1
			if full:
				#print(board.toString())
				var toCheck = ""
				for x in range(len(board.state)):
					toCheck += board.state[x][y][1]
...
func chooseBoard():
	...
	for i in range(len(potentialBoards)):
		if potentialBoards[i].lineScore > bestLineScore:
			bestLineScore = potentialBoards[i].lineScore
			bestHoles = potentialBoards[i].holeCount
			bestHighest = potentialBoards[i].highestPoint
			chosen = i
		elif potentialBoards[i].lineScore == bestLineScore and potentialBoards[i].holeCount < bestHoles:
			bestHoles = potentialBoards[i].holeCount
			bestHighest = potentialBoards[i].highestPoint
			chosen = i
		elif potentialBoards[i].lineScore == bestLineScore and potentialBoards[i].holeCount == bestHoles and potentialBoards[i].highestPoint > bestHighest:
			bestHighest = potentialBoards[i].highestPoint
			chosen = i
	#print("----------------- Chosen -----------------")
